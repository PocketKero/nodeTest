...
module.exports = (uploadDir) => {
	...
	dirNames.forEach((dirName) => {
		...
		try {
			if (fs.statSync(path.resolve(uploadDir, dirName, 'entry_sheet.csv'))) {
				// console.log(dirName);
				...
				if (fileName == dirName && fs.statSync(path.resolve(uploadDir, dirName, fileName+fileExt)) && fileExt == ext) {
					// console.log("　　あってます");
				} else {
					if (fileName != dirName) {
